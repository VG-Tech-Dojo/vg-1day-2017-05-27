...
func MessagesAll(db *sql.DB) ([]*Message, error) {
	...
	for rows.Next() {
		m := &Message{}
		// 1-1. ユーザー名を表示しよう
		if err := rows.Scan(&m.ID, &m.Body, &m.Uname); err != nil {
			return nil, err
		}
		ms = append(ms, m)
	}
...
func MessageByID(db *sql.DB, id string) (*Message, error) {
	...
	// 1-1. ユーザー名を表示しよう
	if err := db.QueryRow(`select id, body, username from message where id = ?`, id).Scan(&m.ID, &m.Body, &m.Uname); err != nil {
		return nil, err
	}
...
func (m *Message) Insert(db *sql.DB) (*Message, error) {
	// 1-2. ユーザー名を追加しよう
	res, err := db.Exec(`insert into message (body, username) values (?, ?)`, m.Body, m.Uname)
	if err != nil {
		return nil, err
	}
